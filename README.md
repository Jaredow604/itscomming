...
| **PyTorch** | Ecosistema de Machine Learning para predicción de Player Props. |
| **PostgreSQL & SQLAlchemy** | Data Warehouse para el almacenamiento atómico de estadísticas. |
| **Django / FastAPI** | Arquitectura del servidor y Endpoints API. |
| **Pandas / NumPy** | Limpieza de datos y normalización vectorizada del modelo. |
| **soccerdata (FBref)** | Extracción especializada de métricas avanzadas y xG mundiales. |
| **Google GenAI** | Procesamiento de lenguaje natural para el análisis experto. |
| **The Odds API** | Análisis y mapeo de la ventaja contra los momios. |

---

## 💾 Data Engineering & Machine Learning Pipeline

Nuestra última actualización robusteció la ingesta permitiéndonos cruzar datos de manera segura:
* **Entity Resolution**: Un sistema capaz de empatar entidades asimétricas (ej. "Guadalajara" y "Chivas") a las mismas Primary Keys en la base de datos relacional.
* **Ingesta Atómica (SQLAlchemy)**: Nuestro `loader.py` funciona como orquestador asincrónico que alimenta a los Data Warehouse satelitales (NBA, MLB, Elite Fútbol, Liga MX) con lógica transaccional.
* **PyTorch Integration**: Los datos recolectados se transforman directamente en un `Dataset` de Pytorch (`fbref_player_dataset.py`) de manera abstracta operando normalización Min-Max Escalada para entregar *Tensors* pulidos a las capas de la Red Neuronal.
